...
import { useAssetLoading } from '../LayoutWrapper'
...
function EarthMesh() {
  ...
  const { setAssetsLoaded } = useAssetLoading()
  ...
  // Notify when textures are loaded
  useEffect(() => {
    if (colorMap && normalMap && aoMap) {
      // Small delay to ensure everything is rendered
      setTimeout(() => {
        setAssetsLoaded(true)
      }, 100)
    }
  }, [colorMap, normalMap, aoMap, setAssetsLoaded])
...
function MultipleImages() {
  const images: OrbitingImageProps[] = [
    { imagePath: "/assets/etsy.jpeg", radius: 2.8, speed: 0.5, offset: 0, size: 0.6 },
    { imagePath: "/assets/netflix.png", radius: 3.2, speed: -0.4, offset: Math.PI / 2, size: 0.5 },
    { imagePath: "/assets/tesla.png", radius: 3.6, speed: 0.3, offset: Math.PI, size: 0.7 },
  ]
