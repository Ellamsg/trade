...
export default function Navbar() {
  ...
  return (
    <nav className={`fixed top-0 w-full bg-gblack transition-all z-[999] duration-300 ${scrolled ? 'bg-black shadow-md py-2' : 'bg-black/90 py-4'}`}>
      <div className="container  mx-auto px-4">
        <div className="flex  justify-between items-center">
          ...
          <div className='md:block hidden'>
            <button className='bg-[#131313] py-2 px-6 rounded-[20px]'>Login</button>
          </div>
          ...
          <div className="md:hidden flex items-center">
            ...
            >
              <div className="w-6 flex flex-col items-end space-y-1.5">
                ...
                <span className={`block h-0.5 bg-gray-800 transition-all duration-300 ${isOpen ? 'opacity-0' : 'w-5'}`}></span>
                <span className={`block h-0.5 bg-gray-800 transition-all duration-300 ${isOpen ? 'w-6 -rotate-45 -translate-y-2' : 'w-4'}`}></span>
              </div>
            </button>
          </div>
        </div>
