...
//   // Fetch stock data from FMP API with revalidation
//   useEffect(() => {
...
//         // Fetch most active stocks (revalidates every 5 minutes)
...
//             setPrice(formattedStocks[0].price.toString());
//           }
//         }
//       } catch (error) {
...
//           totalValue: totalAmount * (selectedAsset?.price || approvedOrder.price)
//         };
//       } else {
...
//           totalValue: approvedOrder.amount * (selectedAsset?.price || approvedOrder.price),
//           addedAt: new Date()
//         };
...
//               <div className="flex items-center justify-between">
//                 <div>
//                   <p className="text-slate-400 text-sm">Approved Orders</p>
//                   <p className="text-2xl font-bold text-white">{approvedOrders.length}</p>
//                 </div>
...
//               <div className="flex items-center justify-between">
//                 <div>
//                   <p className="text-slate-400 text-sm">Active Stocks</p>
//                   <p className="text-2xl font-bold text-white">{new Set(orders.map(o => o.asset)).size}</p>
//                 </div>
...
//       {/* New Order Modal */}
//       {isModalOpen && (
//         <div className="fixed text-[14px] inset-0 z-50 flex items-center justify-center p-4 overflow-y-auto">
//           <div className="absolute inset-0 bg-black/20 backdrop-blur-md" onClick={() => setIsModalOpen(false)}></div>
//           <div className="relative bg-slate-900/95 backdrop-blur-xl rounded-2xl p-8 w-full max-w-lg border border-slate-600/50 shadow-2xl max-h-[90vh] overflow-y-auto">
//             <div className="flex justify-between items-center mb-6">
//               <h2 className="text-[16px] font-bold bg-gradient-to-r from-white to-slate-200 bg-clip-text text-transparent">
...
//                       {stock.symbol} - {stock.name} (${stock.price.toLocaleString()})
//                     </option>
//                   ))}
//                 </select>
//               </div>
...
//                         selectedAsset.changesPercentage >= 0 ? 'text-green-400' : 'text-red-400'
//                       }`}>
//                         {selectedAsset.changesPercentage >= 0 ? '+' : ''}
//                         {selectedAsset.changesPercentage.toFixed(2)}%
//                       </div>
//                     </div>
//                   </div>
//                 </div>
...
//               // Add to portfolio when approved
//               addToPortfolio(approvedOrder);
//               updateOrderInSupabase(approvedOrder);
...
//   const addToPortfolio = async (approvedOrder: Order) => {
//     if (approvedOrder.type === 'buy') {
//       // Get existing portfolio from Supabase
//       const { data: existingPortfolio, error } = await supabase
//         .from('portfolio')
//         .select('*')
//         .eq('asset', approvedOrder.asset);
...
//       const existingItem = existingPortfolio?.[0];

//       if (existingItem) {
//         // Update existing asset
//         const totalAmount = existingItem.amount + approvedOrder.amount;
//         const totalValue = (existingItem.amount * existingItem.average_price) + (approvedOrder.amount * approvedOrder.price);
//         const newAveragePrice = totalValue / totalAmount;
...
//             total_value: totalAmount * (selectedAsset?.current_price || approvedOrder.price),
...
//           asset: approvedOrder.asset,
//           asset_name: approvedOrder.assetName,
//           amount: approvedOrder.amount,
//           average_price: approvedOrder.price,
//           total_value: approvedOrder.amount * (selectedAsset?.current_price || approvedOrder.price),
