...
//           console.error('Error fetching pending transaction:', transactionError);
//         }

//         if (transactionData) {
//           setAccountRequest(transactionData);
//           if (transactionData.account_number) {
//             setWaitingForAccount(false);
//             createWallet(transactionData.wallet_type, transactionData.account_number, transactionData.network, transactionData.token_type);
//           } else {
...
//         if (upgradeError) {
//           console.error('Error fetching upgrade requests:', upgradeError);
//         } else {
...
//         schema: 'public', 
//         table: 'wallets', 
//         filter: `wallet_number=eq.${wallet.wallet_number}`
//       }, (payload) => {
...
//       }, (payload) => {
//         const data = payload.new as TransactionRequest;
//         setAccountRequest(data);
//         if (data.account_number) {
//           supabase.removeChannel(channelRef.current);
//           channelRef.current = null;
//           setWaitingForAccount(false);
//           createWallet(data.wallet_type, data.account_number, data.network as NetworkType, data.token_type as TokenType);
//         }
//       })
//       .subscribe((status) => {
//         if (status === 'CHANNEL_ERROR') {
//           console.error('Realtime subscription for transaction failed');
...
//   const [isUpgradesLoading, setIsUpgradesLoading] = useState(true);
...
//   const supabase = createClient();
//   const pollingIntervalRef = useRef<NodeJS.Timeout | null>(null);
//   const walletPollingRef = useRef<NodeJS.Timeout | null>(null);
...
//       if (walletPollingRef.current) clearInterval(walletPollingRef.current);
...
//       }
//       if (upgradeRequests.some(req => !req.status)) {
...
//         const updatedWallet = payload.new as UserWallet;
//         setWallet(updatedWallet);
//         if (updatedWallet.status === true && walletPollingRef.current) {
//           clearInterval(walletPollingRef.current);
//           walletPollingRef.current = null;
//         }
//       })
...
//       }, async (payload) => {
//         console.log('Upgrade request update received:', payload);
//         await fetchUpgradeRequests(true); // Silent fetch
//         await fetchWalletData(true); // Silent fetch
//         if (payload.new.status === true && upgradePollingRef.current) {
//           clearInterval(upgradePollingRef.current);
//           upgradePollingRef.current = null;
//         }
//       })
...
//         filter: `wallet_id=eq.${wallet.id}`
//       }, async () => {
//         await fetchWithdrawals(true); // Silent fetch
//       })
//       .subscribe((status) => {
...
//   const startWalletStatusPolling = () => {
//     if (walletPollingRef.current) {
//       clearInterval(walletPollingRef.current);
//     }

//     walletPollingRef.current = setInterval(async () => {
//       console.log('Polling wallet status...');
//       await fetchWalletData(true); // Silent fetch
//     }, 5000);
//   };
...
//       console.log('Polling upgrade requests...');
//       await fetchUpgradeRequests(true); // Silent fetch
//       await fetchWalletData(true); // Silent fetch
//     }, 5000);
//   };
...
//   const fetchUpgradeRequests = async (silent = false) => {
//     try {
//       if (!silent) setIsUpgradesLoading(true);
//       const { data: { user } } = await supabase.auth.getUser();
...
//       if (error) throw error;
//       setUpgradeRequests(data || []);
//       if (data.every(req => req.status === true) && upgradePollingRef.current) {
//         clearInterval(upgradePollingRef.current);
//         upgradePollingRef.current = null;
//       }
//     } catch (error) {
//       console.error('Error fetching upgrade requests:', error);
//     } finally {
//       if (!silent) setIsUpgradesLoading(false);
//     }
//   };

//   const fetchWithdrawals = async (silent = false) => {
//     try {
//       if (!silent) setIsWithdrawalsLoading(true);
//       const { data: { user } } = await supabase.auth.getUser();
//       if (!user || !wallet) return;
...
//       console.error('Error fetching withdrawals:', error);
//     } finally {
//       if (!silent) setIsWithdrawalsLoading(false);
//     }
//   };
...
//     try {
//       if (!silent) setIsWalletLoading(true);
//       const { data: { user } } = await supabase.auth.getUser();
...
//       } else if (walletData) {
//         setWallet(walletData);
//         if (walletData.status === true && walletPollingRef.current) {
//           clearInterval(walletPollingRef.current);
//           walletPollingRef.current = null;
//         }
//       }
...
//       console.error('Error fetching wallet data:', error);
//     } finally {
//       if (!silent) setIsWalletLoading(false);
//     }
//   };
...
//   if (isWalletLoading && !wallet && !upgradeRequests.length && !withdrawals.length && !stockPortfolio.length) {
//     return (
//       <div className="min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 text-white flex items-center justify-center">
//         <div className="text-center">
//           <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-blue-400 mx-auto mb-4"></div>
...
//                     <p className="text-sm text-slate-400">
//                       Upgrade to {TIER_CONFIG[req.target_tier].name}: Please send 
//                       <span className="font-bold text-white"> ${(TIER_CONFIG[req.target_tier].minimum - wallet!.balance).toLocaleString()}</span> to account: 
//                       <span className="font-mono font-bold text-white ml-2">{wallet?.wallet_number}</span>
//                     </p>
//                     <p className="text-sm text-slate-400 mt-1">
//                       Network: {wallet?.network && NETWORK_CONFIG[wallet.network].name} • 
...
//         <div className="bg-gradient-to-r from-slate-800/50 to-slate-900/50 backdrop-blur-sm rounded-2xl p-6 mb-6 md:mb-8 border border-slate-700/50 shadow-2xl relative">
...
//           <div className="mt-8">
//             <h3 className="text-xl font-bold mb-4">Pending Tier Upgrades</h3>
//             <div className="bg-slate-800/50 rounded-xl border border-slate-700/50 overflow-hidden relative">
//               {isUpgradesLoading && upgradeRequests.length === 0 && (
//                 <div className="p-8 text-center">
//                   <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-400 mx-auto mb-4"></div>
//                   <p className="text-slate-400">Loading upgrades...</p>
//                 </div>
//               )}
//               {isUpgradesLoading && upgradeRequests.length > 0 && (
//                 <div className="absolute top-4 right-4">
