...
const PortfolioPage = () => {
  ...
  useEffect(() => {
    if (wallet) {
      setupRealtimeSubscriptions();
      // Start polling if wallet is pending (status: false)
      if (wallet.status === false) {
        startWalletStatusPolling();
      }
      // Start polling for upgrade requests if there are pending ones
      if (upgradeRequests.some(req => !req.status)) {
        startUpgradeStatusPolling();
      }
    } else {
      // Clear polling if no wallet exists
      if (walletPollingRef.current) {
        clearInterval(walletPollingRef.current);
  ...
  const setupRealtimeSubscriptions = async () => {
    ...
    channelRef.current = supabase
      ...
      .on('postgres_changes', { 
        event: 'UPDATE', // Narrow to UPDATE for wallet_upgrades
        schema: 'public', 
        table: 'wallet_upgrades', 
        ...
        await fetchWalletData(); // Refresh wallet for balance/tier updates
  ...
  const startUpgradeStatusPolling = () => {
    ...
    upgradePollingRef.current = setInterval(async () => {
      ...
      await fetchUpgradeRequests();
      await fetchWalletData(); // Ensure wallet balance/tier updates
    }, 5000);
  };
  ...
  const fetchUpgradeRequests = async () => {
    try {
      ...
      if (error) throw error;
      setUpgradeRequests(data || []);
      // Stop polling if no pending upgrades
  ...
  const handleTierUpgrade = async () => {
    ...
    try {
      ...
      setShowUpgradeForm(false);
      setSelectedUpgradeTier(null);
      startUpgradeStatusPolling(); // Start polling after new request
    } catch (error) {
      console.error('Error requesting tier upgrade:', error);
      alert('Failed to request upgrade. Please try again.');
    } finally {
